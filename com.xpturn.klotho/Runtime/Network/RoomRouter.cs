...
namespace xpTURN.Klotho.Network
{
    ...
    public class RoomRouter
    {
        ...
        public RoomRouter(INetworkTransport transport, IKLogger logger)
        {
        ...
        private void RouteFirstMessage(int peerId, byte[] data, int length)
        {
            ...
            if (msg == null)
            {
                _logger?.KWarning($"[RoomRouter] Peer {peerId}: first message is not RoomHandshakeMessage, disconnecting");
            ...
            _logger?.KInformation($"[RoomRouter] Peer {peerId} → Room {roomId}");
        }
        ...
        private Room ValidateAndResolveRoom(int peerId, int roomId)
        {
            if (!_rooms.TryGetValue(roomId, out Room room))
            {
                ...
                if (room == null)
                {
                    _logger?.KWarning($"[RoomRouter] Peer {peerId}: room {roomId} not found");
            ...
            {
                _logger?.KWarning($"[RoomRouter] Peer {peerId}: room {roomId} is {room.State}");
            ...
            {
                _logger?.KWarning($"[RoomRouter] Peer {peerId}: room {roomId} is ending (reason={room.EndReason}, grace pending)");
            ...
            {
                _logger?.KWarning($"[RoomRouter] Peer {peerId}: room {roomId} is full");
        ...
        private void RejectAndDisconnect(int peerId, byte reason)
        {
            ...
            // Carry the reason on the disconnect packet itself; a separate reliable message would be
            // dropped because the immediately following disconnect halts the peer's send flush.
        ...
        public void CleanupUnroutedPeers()
        {
            ...
            if (expired != null)
            {
                for (int i = 0; i < expired.Count; i++)
                {
                    ...
                    _logger?.KWarning($"[RoomRouter] Unrouted peer {peerId} timed out, disconnecting");
                    _unroutedPeers.Remove(peerId);
                    _transport.DisconnectPeer(peerId);
                }
            }
