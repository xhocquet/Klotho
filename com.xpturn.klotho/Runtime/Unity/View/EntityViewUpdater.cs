    ...
    public class EntityViewUpdater : MonoBehaviour
    {
        ...
        private void TrySpawn(EntityRef entity, FrameRef frame, BindBehaviour behaviour)
        {
            ...
            if (_viewsByEntityIndex.TryGetValue(entity.Index, out var existing))
            {
                ...
                Engine?.Logger?.KDebug($"[ViewLife][Rebind] entity={entity.Index}, viewType={existing.GetType().Name}, oldVersion={existing.EntityRef.Version}, newVersion={entity.Version}, versionMatch={versionMatch}, ownerMatch={ownerMatch}, viewIID={existing.GetInstanceID()}");
        ...
        private void DestroyStale()
        {
            ...
            foreach (var idx in _staleIndices)
            {
                ...
                Engine?.Logger?.KDebug($"[ViewLife][StaleDestroy] entity={idx}, viewType={view.GetType().Name}, viewVersion={view.EntityRef.Version}, viewIID={view.GetInstanceID()}");
                view.OnDeactivate();
