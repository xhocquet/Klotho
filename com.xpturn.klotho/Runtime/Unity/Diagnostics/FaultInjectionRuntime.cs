    ...
    public sealed class FaultInjectionRuntime
    {
        ...
        void UpdateDisconnectSchedule()
        {
            ...
            while (_disconnectNextIdx < schedule.Count && anchorElapsed >= schedule[_disconnectNextIdx].atSec)
            {
                ...
                {
                    _logger?.KWarning($"[FaultInjection] Disconnect peer={entry.playerId.Value} at anchorSec={entry.atSec:F1} duration={entry.durationSec:F1}s (tick={currentTick})");
                ...
                else if (_transport != null)
                {
                    ...
                    {
                        _logger?.KWarning($"[FaultInjection] Disconnect peer={peerId} at anchorSec={entry.atSec:F1} duration={entry.durationSec:F1}s (tick={currentTick})");
            ...
            {
                _disconnectReconnectAt = -1f;
                _logger?.KWarning($"[FaultInjection] Reconnect trigger after disconnect (tick={currentTick})");
