    ...
    public partial class KlothoNetworkService
    {
        ...
        private void HandleSpectatorJoin(int peerId, SpectatorJoinMessage msg)
        {
            ...
            if (snapshotTick >= 0)
            {
                ...
                {
                    _logger?.KWarning($"[KlothoNetworkService][HandleSpectatorJoin] Cannot serialize input range for spectator peer {peerId}");
                    return;
                }
                if (verifyData != null)
        ...
        private void HandleFrameVerifiedForCatchup(int verifiedTick)
        {
            ...
            foreach (var kvp in _lateJoinCatchups)
            {
                ...
                if (!_engine.TrySerializeVerifiedInputRange(fromTick, toTick, out byte[] rangeData, out int len))
                {
                    ...
                    {
                        _logger?.KWarning($"[KlothoNetworkService][LateJoinCatchup] Serialize failed: peer={info.PeerId}, fromTick={fromTick}, toTick={toTick}, failCount={failCount} — possible OldestTick race; guest may need FullState resync");
                    }
                    continue;
                }
            ...
            for (int i = 0; i < _completedLateJoinCatchups.Count; i++)
            {
                ...
                _logger?.KInformation($"[KlothoNetworkService][LateJoinCatchup] Catchup complete: peerId={peerId}, verifiedTick={verifiedTick}");
            }
            _completedLateJoinCatchups.Clear();
        }
        ...
        private void HandleVerifiedInputBatchReady(int startTick, int tickCount, byte[] data, int dataLength)
        {
            ...
            //_logger?.KInformation($"[KlothoNetworkService][BatchReady] startTick={startTick}, tickCount={tickCount}, batchEnd={batchEnd}, spectators={_spectators.Count}");
            ...
            using (var serialized = _messageSerializer.SerializePooled(msg))
            {
                for (int i = 0; i < _spectators.Count; i++)
                {
                    ...
                    {
                        _logger?.KInformation($"[KlothoNetworkService][BatchReady] Spectator {spectator.PeerId} skipped: LastSentTick={spectator.LastSentTick} >= batchEnd={batchEnd}");
                        continue;
                    }
                    if (spectator.LastSentTick + 1 == startTick)
                    {
                        //_logger?.KInformation($"[KlothoNetworkService][BatchReady] SEND direct to {spectator.PeerId}: ticks {startTick}-{batchEnd}, serializedLen={serialized.Length}");
                    ...
                    {
                        _logger?.KInformation($"[KlothoNetworkService][BatchReady] Spectator {spectator.PeerId} gap detected: LastSentTick={spectator.LastSentTick}, startTick={startTick} → catchup");
        ...
        private void SendSpectatorCatchupInputs(SpectatorInfo spectator)
        {
            ...
            if (fromTick > toTick)
            {
                _logger?.KInformation($"[KlothoNetworkService][Catchup] Skipped: fromTick={fromTick} > toTick={toTick}");
                return;
            }

            if (!_engine.TrySerializeVerifiedInputRange(fromTick, toTick, out byte[] inputData, out int dataLen))
            {
                _logger?.KWarning($"[KlothoNetworkService][Catchup] Serialization failed: fromTick={fromTick}, toTick={toTick}");
                return;
            }
            ...
            _logger?.KInformation($"[KlothoNetworkService][Catchup] Sent: {spectator.PeerId}, ticks {fromTick}-{toTick}, bytes={dataLen}");
            using (var serialized = _messageSerializer.SerializePooled(msg))
                _transport.Send(spectator.PeerId, serialized.Data, serialized.Length, DeliveryMethod.ReliableOrdered);
