...
namespace xpTURN.Klotho.Core
{
    ...
    public class DedicatedServerLoop
    {
        ...
        public DedicatedServerLoop(IKlothoEngine engine, int tickIntervalMs, IKLogger logger)
            : this(engine, null, tickIntervalMs, logger, new CancellationTokenSource())
        {
        }

        public DedicatedServerLoop(IKlothoEngine engine, INetworkTransport transport, int tickIntervalMs, IKLogger logger)
            : this(engine, transport, tickIntervalMs, logger, new CancellationTokenSource())
        {
        }

        public DedicatedServerLoop(IKlothoEngine engine, int tickIntervalMs, IKLogger logger, CancellationTokenSource cts)
            : this(engine, null, tickIntervalMs, logger, cts)
        {
        }

        public DedicatedServerLoop(IKlothoEngine engine, INetworkTransport transport, int tickIntervalMs, IKLogger logger, CancellationTokenSource cts)
        {
            _logger = logger;
